    ...
    <div className="flex h-[100dvh] flex-col bg-zinc-50 text-zinc-900 dark:bg-zinc-950 dark:text-zinc-50">
      <header className="flex shrink-0 items-center justify-between border-b border-zinc-200 px-4 py-3 dark:border-zinc-800">
        <div>
          ...
          <p className="text-xs text-zinc-500 dark:text-zinc-400">
            Session{' '}
            <code className="rounded bg-zinc-200 px-1 py-0.5 text-[11px] dark:bg-zinc-800">
              {shortSession}
            </code>
          </p>
        </div>
      ...
      <div className="min-h-0 flex-1 overflow-y-auto px-4 py-4">
        {messages.length === 0 && !loading && (
          <p className="mx-auto max-w-xl rounded-xl border border-dashed border-zinc-300 bg-white/80 p-6 text-center text-sm text-zinc-600 dark:border-zinc-700 dark:bg-zinc-900/50 dark:text-zinc-300">
            Say hi — your messages stay in this browser for this session. The
            same{' '}
            <span className="font-medium text-zinc-800 dark:text-zinc-100">
              session id
            </span>{' '}
            is sent to the API so memory + personalization can line up with
            the backend.
          </p>
        )}
      ...
      >
        <div className="mx-auto flex max-w-3xl gap-2">
          <input
            className="min-h-11 flex-1 rounded-xl border border-zinc-300 bg-white px-3 text-sm outline-none ring-zinc-400 focus:ring-2 dark:border-zinc-600 dark:bg-zinc-900"
